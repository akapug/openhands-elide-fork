...
const RUN_ID = process.env.BENCH_RUN_ID || new Date().toISOString().replace(/[:.]/g,'-')
...
function runSweep(name: string, baseURL: string, out: string, extraEnv: Record<string, string> = {}) {
  return new Promise<void>((resolve, reject) => {
    const conc = (extraEnv.BENCH_CONCURRENCY || process.env.BENCH_CONCURRENCY || '8')
    const total = (extraEnv.BENCH_TOTAL || process.env.BENCH_TOTAL || '64')
...
async function waitHealthy(name: string, url: string, timeoutMs = Number(process.env.BENCH_HEALTH_TIMEOUT_MS || 60000)) {
  const t0 = Date.now()
...
async function killListenersOnWindows(port: number): Promise<void> {
  ...
  const auto = String(process.env.BENCH_KILL_CONFLICTS || '1').toLowerCase()
...
function parseTiers() {
  const concList = (process.env.BENCH_CONCURRENCY_LIST || '').split(',').map(s=>s.trim()).filter(Boolean).map(Number)
  const totalList = (process.env.BENCH_TOTAL_LIST || '').split(',').map(s=>s.trim()).filter(Boolean).map(Number)
  if (concList.length && totalList.length && concList.length === totalList.length) {
    return concList.map((c,i)=>({ c, t: totalList[i] }))
  }
  const c = Number(process.env.BENCH_CONCURRENCY || '8')
  const t = Number(process.env.BENCH_TOTAL || '64')
  return [{ c, t }]
}

function parseTargets(): Set<string> {
  const def = ['node-raw','elide','express','fastapi','flask']
  const raw = String(process.env.BENCH_TARGETS || '')
...
async function main() {
  const sequential = String(process.env.BENCH_MODE || '').toLowerCase() === 'sequential'
  const tiers = parseTiers()

  const targets = parseTargets()
  const startAllFlag = String(process.env.BENCH_START_SERVERS || '').toLowerCase()
  ...
  const baseNodeRaw = process.env.BENCH_BASE_NODE_RAW || 'http://localhost:8080'
  const baseExpress = process.env.BENCH_BASE_EXPRESS || 'http://localhost:8081'
  const baseFastapi = process.env.BENCH_BASE_FASTAPI || 'http://localhost:8082'
  const baseFlask   = process.env.BENCH_BASE_FLASK   || 'http://localhost:8083'
  const baseElide   = process.env.BENCH_BASE_ELIDE   || 'http://localhost:8084'


  // Optional: dockerized Flask
  const useDockerFlask = (()=>{ const f=String(process.env.BENCH_DOCKER_FLASK||'').toLowerCase(); return f==='1'||f==='true' })()
  ...
  if (startAll && targets.has('node-raw')) {
    ...
    if (process.platform === 'win32' && (String(process.env.BENCH_WSL_NODE||'').toLowerCase()==='1' || String(process.env.BENCH_WSL_NODE||'').toLowerCase()==='true')) {
  ...
  if (startAll && targets.has('elide')) {
    ...
    const cmd = String(process.env.BENCH_ELIDE_CMD || '').trim()
    if (cmd) {
      if (process.platform === 'win32') {
      ...
      process.stderr.write('[elide][err] BENCH_ELIDE_CMD not provided; will only use base URL if already running\n')
    }
  }
  ...
  if (startAll && targets.has('fastapi')) {
    if (process.platform === 'win32' && (String(process.env.BENCH_WSL_FASTAPI||'').toLowerCase()==='1' || String(process.env.BENCH_WSL_FASTAPI||'').toLowerCase()==='true')) {
      const rootWsl = winPathToWsl(repoRoot())
  ...
  try {
    ...
    const runForTier = async (tier: {c:number,t:number}) => {
      const baseEnv = { ...process.env }
      const envNodeRaw  = { ...baseEnv, BENCH_CONCURRENCY: String(tier.c), BENCH_TOTAL: String(tier.t), LLM_MODEL: 'synthetic' }
      const envElide    = { ...baseEnv, BENCH_CONCURRENCY: String(tier.c), BENCH_TOTAL: String(tier.t), LLM_MODEL: 'synthetic' }
      const envExpress  = { ...baseEnv, BENCH_CONCURRENCY: String(tier.c), BENCH_TOTAL: String(tier.t) }
      const envFastapi  = { ...baseEnv, BENCH_CONCURRENCY: String(tier.c), BENCH_TOTAL: String(tier.t) }
      const envFlask    = { ...baseEnv, BENCH_CONCURRENCY: String(tier.c), BENCH_TOTAL: String(tier.t) }
